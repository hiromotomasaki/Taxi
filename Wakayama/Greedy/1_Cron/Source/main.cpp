		...
		// : infoAboutValidCells/(indexTaxi)/searchRange/dir(k).xml
		{
			...
			for (int i = 0; i < numCell; i++) {
				...
				if (vValid[i]) {
					...
					{
						{
							std::string fileDire = "./../Data/0_2_Set/infoAboutValidCells/" + boost::lexical_cast<std::string>( indexTaxi );
							boost::filesystem::path path(fileDire);
							boost::system::error_code error;
							const bool result = boost::filesystem::create_directories(path, error);
							if (!result || error) {
						...
						{
							std::string fileDire = "./../Data/0_2_Set/infoAboutValidCells/" + boost::lexical_cast<std::string>( indexTaxi ) + "/object";
							boost::filesystem::path path(fileDire);
							boost::system::error_code error;
							const bool result = boost::filesystem::create_directories(path, error);
							if (!result || error) {
						...
						{
							std::string fileDire = "./../Data/0_2_Set/infoAboutValidCells/" + boost::lexical_cast<std::string>( indexTaxi ) + "/object/" + boost::lexical_cast<std::string>( indexTaxi );
							boost::filesystem::path path(fileDire);
							boost::system::error_code error;
							const bool result = boost::filesystem::create_directories(path, error);
							if (!result || error) {
						...
						{
							std::string fileDire = "./../Data/0_2_Set/infoAboutValidCells/" + boost::lexical_cast<std::string>( indexTaxi ) + "/searchRange";
							boost::filesystem::path path(fileDire);
							boost::system::error_code error;
							const bool result = boost::filesystem::create_directories(path, error);
							if (!result || error) {
							...
							if (isSameCell) {
								...
								for (int k = 1; k <= 9; k++) {
									...
									// 保存
									{
										...
										// 保存ファイル先のディレクトリのmakefileからの相対位置
										std::string fileDire = "./../Data/0_2_Set/infoAboutValidCells/" + boost::lexical_cast<std::string>( indexTaxi ) + "/object/" + boost::lexical_cast<std::string>( indexTarget );
										// 保存path
										std::string fileRela = fileDire + "/" + fileName;
										// create an empty property tree
										boost::property_tree::ptree pt;
